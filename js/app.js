...
const App = {
    ...
    showSchoolDetail(schoolId) {
        ...
        document.getElementById('detailPredicted').textContent = school.prediction.priorities.nv1.predicted.toFixed(2);
        document.getElementById('detailRange').textContent = `${school.prediction.priorities.nv1.low.toFixed(2)} - ${school.prediction.priorities.nv1.high.toFixed(2)}`;
        document.getElementById('detailConfidence').textContent = `${school.prediction.confidence}%`;
        const priorities = school.prediction.priorities;
        const nvThresholdsEl = document.getElementById('detailNvThresholds');
        if (nvThresholdsEl) {
            nvThresholdsEl.innerHTML = `
                <div class="stat-card">
                    <div class="stat-label">NV1</div>
                    <div class="stat-value">${priorities.nv1.predicted.toFixed(2)}</div>
                </div>
                <div class="stat-card">
                    <div class="stat-label">NV2</div>
                    <div class="stat-value">${priorities.nv2.predicted.toFixed(2)}</div>
                </div>
                <div class="stat-card">
                    <div class="stat-label">NV3</div>
                    <div class="stat-value">${priorities.nv3.predicted.toFixed(2)}</div>
                </div>
            `;
        }
